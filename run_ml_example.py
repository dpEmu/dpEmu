...
def load_digits_to_npy():
    digits = load_digits()
    path_to_data = generate_unique_path("data", "digits_data") + ".npy"
    path_to_labels = generate_unique_path("data", "digits_labels") + ".npy"
...
def load_mnist_to_npy():
    ...
    path_to_data = generate_unique_path("data", "mnist_data") + ".npy"
    path_to_labels = generate_unique_path("data", "mnist_labels") + ".npy"
...
def generate_unique_path(folder_name, file_name=None):
    current_folder = os.path.dirname(os.path.realpath(__file__))
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S-%f")
    if file_name:
        return os.path.join(current_folder, "{}/{}".format(folder_name, file_name))
    else:
        return os.path.join(current_folder, "{}/{}".format(folder_name, timestamp))
...
def main():
    ...
    path_to_kmeans_model_params = generate_unique_path("tmp") + ".json"
    ...
    path_to_clustering_analyzer_params = generate_unique_path("tmp") + ".json"
