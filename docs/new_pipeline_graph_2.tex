...
\begin{tikzpicture}[state/.style={circle, draw, align=center, minimum size=1cm, text width=1.8cm},file/.style={rectangle, fill=lightgray, draw, align=center, minimum size=1cm, text width=2.5cm}]
	...
	\node[state, right of=errdat, xshift=3cm, yshift=-1cm] (preproc) {Preprocessor}; %
	\node[file, above of=preproc, yshift=1.5cm] (eppdat) {Erroneus preprocessed data}; %
	\node[state, above of=eppdat, yshift=1.5cm] (models) {Models}; %
	\node[file, above of=models, yshift=1.5cm] (results) {Results}; %
	\node[state, right of=runner, xshift=3cm] (visual) {Visualization}; %
	...
	\draw [->] (errdat) |- (preproc) node [near end, fill = white] {$2$};
